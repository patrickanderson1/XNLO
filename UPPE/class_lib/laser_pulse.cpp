...
laser_pulse::laser_pulse(double p_av_, double rep_, double fwhm_, double l_0_, double ceo_,
    ...
    double E_pk = std::sqrt((4 * p_pk) / (physics.c * physics.eps_0 * maths.pi *
                                          (std::pow(waist_, 2))));
    ArrayXd env_t = (-2.77 * (tw.t / fwhm_).pow(2)).exp();
    ArrayXd carr_t = (((2 * maths.pi * physics.c / l_0_) * tw.t) + ceo_).cos();
    
    // Spatial
    ArrayXd env_r = (-(rkr.r / waist_).pow(2)).exp();
...
//------------------------------------------------------------------------------------------------//
// Propagate sprectral amplitudes A_w_active over the longitudinal step dz_,
// based on A. Couairon, et al., Eur. Phys. J. Special Topics, 199, 5 (2011)
void laser_pulse::propagate(double dz_, capillary_fibre capillary_, keldysh_gas gas_) {
    ...
    double z = 0; bool last_step = false;
...
ArrayXXcd laser_pulse::RHS_UPPE(double dz_, ArrayXXcd temp_1, capillary_fibre capillary_,
    ...
    for (int ii = 0; ii < rkr.n_r; ii++) {
        ...
        J_w_active_r.col(ii) = gas_.current_density(temp_3.col(ii).real());
    }
    for (int ii = 0; ii < tw.n_active; ii++) {
